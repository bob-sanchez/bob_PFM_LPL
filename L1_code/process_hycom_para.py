...
def time_filter(in_dir, h_list, out_dir,datestring_start,datestring_end):
    ...
    ds_fmt_H = '%Y.%m.%d.%H'
    ds_fmt = '%Y.%m.%d'
    dts_ini =datestring_start #set intial time
    dtg_ini = datetime.strptime(dts_ini, ds_fmt_H)

    dts_fin =datestring_end #set intial time
    dtg_fin = datetime.strptime(dts_fin, ds_fmt_H)
